...
with plt.rc_context({"figure.figsize": (8, 8)}):
    sc.pl.umap(
        adata,
        color=["cell_type_coarse", "origin", "condition"],
        legend_fontsize=12,
...
# %%
adata = adata[
    (adata.obs["condition"].isin(["LUAD", "LSCC", "NSCLC"]))
    & (
        adata.obs["origin"].isin(
            [
                "normal",
                "normal_adjacent",
                "tumor_edge",
...
fig.savefig("./umap_ihet_on_data.pdf", bbox_inches="tight")
