...
ici_meta = read_tsv("../../data/14_ici_treatment/Genentech_for_scissor/genentech_clinical_data.tsv") |> rename(dataset=study) |> rename(response_category=response, response=response_to_ici) |> mutate(response=factor(response))
...
neutro_sig_tab = read_csv("../../data/30_downstream_analyses/neutrophils/neutrophil_analysis/artifacts/neutro_sigs.csv")
...
lapply(c("LUAD", "LUSC", "all"), function(type) {
  filter_type = if(type == "all") {c("LUAD", "LUSC") } else {type}
  lapply(c("sig_neutro", "sig_nan", "sig_tan", "sig_b_cell_follicular", "sig_neutro2", "sig_neutro3", "sig_lcam_hi", "sig_lcam_consensus"), function(sig) {
    p = plot_kaplan_meyer(
      tcga_clin_and_scores |> filter(type %in% filter_type),
      sig,
...
coxph_results = lapply(c("any", "Docetaxel", "MPDL3280A"), function(arm) {
    lapply(c("all", "LUSC", "LUAD"), function(type) {
        ...
        type_covar = ifelse(type == "all", "+ type ", "")
        coxph_res = lapply(colnames(select(chemo_data, starts_with("sig_"), starts_with("ct_"))), function(col) {
           res = summary(coxph(as.formula(sprintf("Surv(time, status) ~ `%s` %s", col, type_covar)), data=ici_clin_and_scores |> 
...
glm_res_ici = lapply(c("all", "LUSC", "LUAD"), function(type) {
    filter_type = if(type == "all") {c("LUAD", "LUSC") } else {type}
    type_covar = ifelse(type == "all", "+ type ", "")
    tmp_res = lapply(colnames(select(ici_clin_and_scores, starts_with("sig_"), starts_with("ct_"))), function(col) {
      res = summary(glm(as.formula(sprintf("response ~ `%s` %s + dataset", col, type_covar)), family="binomial", data=ici_clin_and_scores |>
                          filter(type %in% filter_type) |>
...
```{r, fig.width=6, fig.height=6}
glm_res_chemo = lapply(c("all", "LUSC", "LUAD"), function(type) {
    filter_type = if(type == "all") {c("LUAD", "LUSC") } else {type}
    type_covar = ifelse(type == "all", "+ type ", "")
    tmp_res = lapply(colnames(select(ici_clin_and_scores, starts_with("sig_"), starts_with("ct_"))), function(col) {
      res = summary(glm(as.formula(sprintf("response_to_chemotherapy ~ `%s` %s + dataset", col, type_covar)), family="binomial", data=ici_clin_and_scores |>
                          filter(type %in% filter_type) |>
                          mutate(response_to_chemotherapy = factor(response_to_chemotherapy))))
      res$coefficients |> as_tibble(rownames="var") |> filter(grepl( col, var)) |> mutate(var=col)
    }) |> 
      bind_rows() |> 
      mutate(sig_code = sig_codes(`Pr(>|z|)`)) |>
...
```{r}
ici_clin_and_scores |> filter(!is.na(response)) |> 
...
ici_clin_and_scores |> filter(!is.na(response)) |> 
  ggplot(aes(y=sig_lcam_consensus, x=sig_neutro2)) + geom_smooth(method="lm", color="black") + geom_point(aes(color=response)) + stat_cor() + theme_cowplot() + background_grid() + scale_color_brewer(palette="Dark2")
...
```{r}
tmp_df = ici_clin_and_scores |> filter(type == "LUSC")
tmp_df$response = as.factor(tmp_df$response)


mod = glm(response ~ sig_neutro2 + sig_lcam_consensus + dataset, data=tmp_df, family=binomial)
summary(mod)

mod2 = glm(response ~ sig_neutro2 + dataset, data=tmp_df, family=binomial)
summary(mod2)

mod3 = glm(response ~ sig_lcam_consensus + dataset, data=tmp_df, family=binomial)
summary(mod3)
...
```{r}
tmp_df = ici_clin_and_scores |> filter(study_arm != "Doxetaxel")
tmp_df$response = as.factor(tmp_df$response)


mod = coxph(Surv(time, status) ~ sig_neutro2 + sig_lcam_consensus + dataset, data=tmp_df)
print(mod)
extractAIC(mod)

mod2 = coxph(Surv(time, status) ~ sig_neutro2 + dataset, data=tmp_df)
print(mod2)
extractAIC(mod2)

mod3 = coxph(Surv(time, status) ~ sig_lcam_consensus + dataset, data=tmp_df)
print(mod3)
extractAIC(mod3)
