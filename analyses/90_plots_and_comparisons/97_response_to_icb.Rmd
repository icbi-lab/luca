...
summary(glm(measure_of_response ~ `ct_T cell CD8+` + type + ajcc_stage, family="binomial", data=chemo_data))

chemo_data |> select(measure_of_response, starts_with("sig_"), starts_with("ct_")) |>
  pivot_longer(-measure_of_response, names_to="signature", values_to="score") |>
  ggplot(aes(x=measure_of_response, y=score)) + 
  geom_quasirandom(aes(color=measure_of_response)) + 
  geom_boxplot(outlier.alpha=0, width=.2) + 
  stat_compare_means(method="wilcox.test") +
  theme_cowplot() + 
  background_grid() + 
  facet_wrap(~signature, ncol = 9) + 
...
tcga_clin_and_scores |> select(ajcc_pathologic_tumor_stage, starts_with("sig_")) |>
  pivot_longer(-ajcc_pathologic_tumor_stage, names_to="signature", values_to="score") |>
  filter(signature == "sig_neutro") |> 
  ggplot(aes(x=ajcc_pathologic_tumor_stage, y=score)) + 
  geom_quasirandom(aes(color=ajcc_pathologic_tumor_stage)) + 
  geom_boxplot(outlier.alpha=0, width=.2, alpha=.8) + 
  theme_cowplot() + 
  background_grid() + 
  ggtitle(sprintf("Scoring function: %s", params$scoring_function))
```
...
ici_deconv = deconvolute(ici_tpm, method="mcp_counter") |> column_to_rownames("cell_type") |> as.matrix() |> t() |> scale() |> as_tibble(rownames="sample") |> rename_at(vars(-sample), function(x) paste0("ct_", x))
  ...
  ici_clin_and_scores = ici_meta |> 
    ...
    mutate(sig_n_to_t_ratio = log2(norm01(sig_neutro)) - log2(norm01(`ct_T cell CD8+`)))
...
ici_clin_and_scores |> select(response, starts_with("sig_"), starts_with("ct_")) |>
    pivot_longer(-response, names_to="signature", values_to="score") |>
    ggplot(aes(x="1", y=score)) + 
    geom_boxplot(outlier.alpha=0, width=.2) + 
    geom_quasirandom(aes(color=response)) + 
    theme_cowplot() + 
    background_grid() + 
    facet_wrap(~signature, ncol = 10, scales = "free_y") + 
    ggtitle(sprintf("Scoring function: %s", params$scoring_function))
```
...
```{r}
lapply(colnames(select(tcga_clin_and_scores, starts_with("sig_"))), function(x) {
  summary(coxph(as.formula(sprintf("Surv(time, status) ~ %s  + age + ajcc_stage + type", x)), data = tcga_clin_and_scores))
})
```

```{r}
lapply(colnames(select(tcga_clin_and_scores, starts_with("sig_"))), function(x) {
  summary(coxph(as.formula(sprintf("Surv(time, status) ~ %s + age + type", x)), data = tcga_clin_and_scores))
})
```
...
plot_kaplan_meyer(tcga_clin_and_scores |> filter(type == "LUAD"), "sig_neutro3")
plot_kaplan_meyer(tcga_clin_and_scores |> filter(type == "LUSC"), "sig_neutro3")
...
ici_clin_and_scores |> select(type, response, starts_with("sig_"), starts_with("ct_")) |>
...
ici_clin_and_scores |> 
  ggplot(aes(y=sig_neutro, x=sig_pd1)) + geom_smooth(method="lm", color="black") + geom_point(aes(color=response)) + stat_cor() + theme_cowplot() + background_grid() + scale_color_brewer(palette="Dark2")
...
ici_clin_and_scores |> 
  ggplot(aes(y=sig_lcam_hi, x=sig_pd1)) + geom_smooth(method="lm", color="black") + geom_point(aes(color=response)) + stat_cor() + theme_cowplot() + background_grid() + scale_color_brewer(palette="Dark2")

ici_clin_and_scores |> 
  ggplot(aes(y=sig_neutro3, x=sig_pd1)) + geom_smooth(method="lm", color="black") + geom_point(aes(color=response)) + stat_cor() + theme_cowplot() + background_grid() + scale_color_brewer(palette="Dark2")

ici_clin_and_scores |> 
  ggplot(aes(y=sig_tan, x=sig_neutro)) + geom_smooth(method="lm", color="black") + geom_point(aes(color=response)) + stat_cor() + theme_cowplot() + background_grid()+ scale_color_brewer(palette="Dark2")
...
ici_clin_and_scores |> 
  ggplot(aes(y=sig_neutro, x=`ct_T cell CD8+`)) + geom_smooth(method="lm", color="black") + geom_point(aes(color=response)) + stat_cor() + theme_cowplot() + background_grid() + scale_color_brewer(palette="Dark2")

ici_clin_and_scores |> 
  ggplot(aes(y=sig_neutro, x=sig_pmn_mdsc)) + geom_smooth(method="lm", color="black") + geom_point(aes(color=response)) + stat_cor() + theme_cowplot() + background_grid() + scale_color_brewer(palette="Dark2")

ici_clin_and_scores |> 
  ggplot(aes(y=sig_neutro, x=sig_plasma)) + geom_smooth(method="lm", color="black") + geom_point(aes(color=response)) + stat_cor() + theme_cowplot() + background_grid() + scale_color_brewer(palette="Dark2")

ici_clin_and_scores |> 
  ggplot(aes(y=sig_lcam_hi, x=sig_plasma)) + geom_smooth(method="lm", color="black") + geom_point(aes(color=response)) + stat_cor() + theme_cowplot() + background_grid() + scale_color_brewer(palette="Dark2")
...
mod = glm(response ~ sig_neutro3 + sig_pd1 + dataset, data=tmp_df, family=binomial)
summary(mod)

mod2 = glm(response ~ sig_neutro3 + dataset, data=tmp_df, family=binomial)
summary(mod2)

mod3 = glm(response ~ sig_pd1 + dataset, data=tmp_df, family=binomial)
summary(mod3)
