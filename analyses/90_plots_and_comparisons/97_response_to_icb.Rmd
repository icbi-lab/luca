...
```{r}
res = summary(coxph(as.formula("Surv(time, status) ~ dataset + type + sig_neutro_balanced * study_arm + gender"), data = ici_clin_and_scores))
res
```
...
lapply(c("sig_neutro_balanced", "sig_tan", "sig_nan", "sig_neutro_balanced_fc"), function(sig) {
...
glm_res_ici = lapply(c("all", "LUSC", "LUAD"), function(type) {
    ...
    tmp_res = lapply(sig_names, function(col) {
      res = summary(glm(as.formula(sprintf("response_to_any ~ `%s` %s + dataset", col, type_covar)), family="binomial", data=ici_data |> filter(type %in% filter_type)))
