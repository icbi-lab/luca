...
```{r}
res = summary(coxph(as.formula("Surv(time, status) ~ type + dataset + sig_nan * study_arm"), data = ici_clin_and_scores))
res
```

```{r}
res = summary(coxph(as.formula("Surv(time, status) ~ sig_tan * type + dataset + sig_tan * study_arm + sig_nan * type + sig_nan * study_arm"), data = ici_clin_and_scores))
res
```
...
```{r}
summary(glm(as.formula("response_to_any ~ sig_tan * type + sig_tan * study_arm + sig_nan * type + dataset + sig_nan * study_arm"), data=ici_clin_and_scores |> filter(!is.na(response_to_any)), family="binomial"))
```
```{r}
...
coxph_results = lapply(c("any", "Docetaxel", "MPDL3280A"), function(arm) {
    lapply(c("all", "LUSC", "LUAD"), function(type) {
        ...
        coxph_res = lapply(sig_names, function(col) {
...
glm_res_ici = lapply(c("all", "LUSC", "LUAD"), function(type) {
    filter_type = if(type == "all") {c("LUAD", "LUSC") } else {type}
    type_covar = ifelse(type == "all", "+ type ", "")
    tmp_res = lapply(sig_names, function(col) {
...
pvalue_df = glm_res_ici |> filter(`Pr(>|z|)` < 0.5, type == "all") |> 
