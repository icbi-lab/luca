...
```{r}
res = summary(coxph(as.formula("Surv(time, status) ~ sig_trn * study_arm + dataset + gender + type"), data = ici_clin_and_scores))
res
```

```{r}
res = summary(coxph(as.formula("Surv(time, status) ~ dataset + type * study_arm"), data = ici_clin_and_scores))
res
```
...
```{r}
summary(glm(as.formula("response_to_any ~ dataset + type * study_arm"), data=ici_clin_and_scores |> filter(!is.na(response_to_any)), family="binomial"))
```
```{r}
summary(glm(as.formula("response_to_any ~ sig_nan * type + dataset + sig_tan * study_arm"), data=ici_clin_and_scores |> filter(!is.na(response_to_any)), family="binomial"))
```
...
lapply(c("any", "Docetaxel", "MPDL3280A"), function(arm) {
    lapply(c("LUAD", "LUSC", "all"), function(type) {
        filter_type = if(type == "all") {c("LUAD", "LUSC") } else {type}
        filter_arm = if(arm == "any") {c("Docetaxel", "MPDL3280A")} else {arm}
        lapply(c("sig_tan", "sig_nan", "sig_trn", "sig_neutro_balanced_fc"), function(sig) {
          p = plot_kaplan_meyer(
...
plot_kaplan_meyer_multi = function(clinical_data, column, q=c(0.1, .9), pvalue=TRUE, ...) {
  tmp_col = clinical_data[[column]]
  cutoffs = quantile(tmp_col, q)
  group = rep(NA, length(tmp_col))
  group[tmp_col < cutoffs[1]] = "bottom 25%"
...
```{r}
ici_clin_and_scores |>  
  ggplot(aes(y=sig_trn, x=sig_neutro_old)) + geom_smooth(method="lm", color="black") + geom_point(aes(color=type)) + stat_cor() + theme_cowplot() + background_grid() + scale_color_brewer(palette="Dark2")
...
```{r}
patient_strat_francesca = read_rds("../../tables/tcga/quantiseq_patient_stratification.rds") |> as.data.frame() |> as_tibble(rownames="patient_id")
colnames(patient_strat_francesca) = c("TCGA_patient_barcode", "patient_strat_francesca")
patient_strat_francesca$TCGA_patient_barcode = str_replace_all(patient_strat_francesca$TCGA_patient_barcode, "(LUAD|LUSC)_TCGA\\.(.{2})\\.(.{4}).*", "TCGA-\\2-\\3")
tcga_clin_and_scores = tcga_clin_and_scores |> left_join(patient_strat_francesca)
```

```{r}
patient_strat_cibersortx = read_csv("/home/sturm/Downloads/stratification_cibersortx.csv")
```
...
```{r}
summary(coxph(as.formula("Surv(time, status) ~ patient_strat_francesca + age + gender"), data=tcga_clin_and_scores))
```

```{r}
summary(coxph(as.formula("Surv(time, status) ~ immune_type + age + gender"), data=patient_strat_cibersortx))
```
...
fit = survfit(Surv(time, status) ~ immune_type, data=patient_strat_cibersortx)
ggsurvplot(fit, data=patient_strat_cibersortx)
```

```{r, fig.width=12, fig.height=8}
fit = survfit(Surv(time, status) ~ patient_strat_francesca, data=tcga_clin_and_scores)
