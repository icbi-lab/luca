...
```{r}
res = summary(coxph(as.formula("Surv(time, status) ~ type + dataset + sig_neutro_top_0 * study_arm"), data = ici_clin_and_scores))
res
```
...
```{r}
res = summary(glm(as.formula("response_to_any ~ type + dataset + sig_neutro_top_0 * study_arm"), data=ici_clin_and_scores |> filter(!is.na(response_to_any)), family="binomial"))
```
...
lapply(c("any", "Docetaxel", "MPDL3280A"), function(arm) {
    lapply(c("LUAD", "LUSC", "all"), function(type) {
        filter_type = if(type == "all") {c("LUAD", "LUSC") } else {type}
        filter_arm = if(arm == "any") {c("Docetaxel", "MPDL3280A")} else {arm}
        lapply(c("sig_neutro_top_0"), function(sig) {
          p = plot_kaplan_meyer(
