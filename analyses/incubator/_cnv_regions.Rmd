...
```{r}
cpdb = read_tsv("~/Downloads/differential_signature_patient_immune_infiltration_cpdb.tsv")
...
```{r}
choose_interactions = function(df, fdr_cutoff = 0.3) {
...
```{r, fig.width=4, fig.height=4}
chr20_interactions |> filter(fdr < 0.5) |>  ggplot(aes(x=group, y=coef)) +  geom_hline(yintercept=0) + geom_quasirandom(aes(color=group)) + geom_boxplot(width=0.2, outlier.alpha = 0)
