...
expressed_genes = read_csv("/home/sturm/Downloads/expressed_genes.csv")
de_tumor_ligands = read_csv("/home/sturm/Downloads/de_tumor_ligands.csv")
cpdb = read_tsv("../../tables/cellphonedb_2022-04-06.tsv")
...
```{r}
luad_lusc_interactions = de_tumor_ligands |> inner_join(cpdb, by=c("gene_id"="source_genesymbol")) |> inner_join(expressed_genes, by=c("target_genesymbol"="variable")) |> select(source=gene_id, target=target_genesymbol, fraction_expressed, fdr=fdr, log2_fc=log2FoldChange, cluster_2=cell_type_major, pvalue) |> mutate(cluster_1 = "Tumor cells") |> mutate(variable=sprintf("%s_%s_%s_%s", source, target, cluster_1, cluster_2), coef=log2_fc)


```

```{r, fig.width=22, fig.height=14, message=FALSE, warning=FALSE, results=FALSE}
luad_lusc_interactions |> mutate(group = if_else(log2_fc < 0, "LUSC", "LUAD")) |> ungroup() |> filter(fraction_expressed>0.1, source %in% de_tumor_ligands$gene_id[1:35]) |> 
  filter(!source %in% c("COPA", "ADGRE5","LGALS9", "NRG1", "SPP1", "JAG1")) |>
  filter(cluster_2 %in% immune_cells) |> cpdb_plot(edge_color="log2_fc") 
```
