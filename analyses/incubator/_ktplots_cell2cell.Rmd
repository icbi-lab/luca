...
cytosig = read_tsv("/home/sturm/Downloads/differential_signature_patient_immune_infiltration_cytosig.tsv")
```
...
cpdb_luad_lscc |> filter(abs(log2_fc) > 1, fdr < 0.2,  cluster_2 == "Neutrophils") |> cpdb_plot(edge_color = "abs(log10(fdr))", edge_color_scale=scale_edge_color_viridis())
```

```{r, fig.width=14, fig.height=8, message=FALSE, warning=FALSE, results=FALSE}
cpdb_luad_lscc |> filter(abs(log2_fc) > 1, cluster_2 == "B cell") |> filter(fdr < 0.33) |> cpdb_plot()
```
...
```{r, fig.width=13, fig.height=7, message=FALSE, warning=FALSE, results=FALSE}
