...
# cpdb_luad_lusc |> filter(abs(log2_fc) > 1, fdr < 0.1,  cluster_2 == "Neutrophils") |> cpdb_plot(edge_color = "abs(log10(fdr))", edge_color_scale=scale_edge_color_viridis())
```


```{r, fig.width=22, fig.height=14, message=FALSE, warning=FALSE, results=FALSE}
cpdb_luad_lusc |> filter(abs(log2_fc) > 1) |> mutate(group2 = if_else(log2_fc < 0, "LUSC", "LUAD")) |> group_by(group2) |>
  filter(row_number() < 30) |> ungroup() |> cpdb_plot(edge_color="coef")
```
...
cpdb_infiltration |> rename(variable=`...1`) |>
  filter(coef > 0, fdr < 0.1, group != "desert") |> 
...
cpdb_infiltration |> rename(variable=`...1`) |>
  mutate(coef = if_else(group == "desert", -coef, coef)) |> 
  filter(coef > 0) |> 
  head(50) |> 
  # filter(cluster_1 %in% c("Tumor cells", "Stromal", "Endothelial cell"), cluster_2 %in% immune_cells ) |>
  # mutate(fdr=p.adjust(pvalue, method="fdr")) |>
  ...
  # filter(row_number() <= 15) |>
  # ungroup() |>
  cpdb_plot(edge_color="coef", edge_color_scale=scale_edge_color_viridis(direction=1, option="inferno"))
```
...
variables = cpdb_infiltration_treatment_condition |> rename(variable=`...1`) |>
  filter(log2_fc > 1) |>
  # filter(cluster_1 %in% c("Tumor cells", "Stromal", "Endothelial cell"), cluster_2 %in% immune_cells ) |>
  group_by(group) |>
  filter(row_number() < 20) |> pull(variable)

cpdb_infiltration_treatment_condition |> rename(variable=`...1`) |>
  mutate(fdr=p.adjust(pvalue, method="fdr")) |>
  filter(log2_fc > 1, fdr < 0.01) |>
...
<!-- ## All checkpoints (LUAD LUSC, tumor immune) -->
<!-- ```{r, fig.width=22, fig.height=14, message=FALSE, warning=FALSE, results=FALSE} -->
<!-- cpdb_luad_lusc |> filter(cluster_1 %in% c("Tumor cells"), target %in% immune_checkpoints$gene_symbol) |>  -->
