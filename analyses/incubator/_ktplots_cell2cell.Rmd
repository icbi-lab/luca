...
cpdb_luad_lusc |> filter(abs(log2_fc) > 1, cluster_2 == "B cell") |> filter(fdr < 0.1) |> cpdb_plot()
```
...
```{r, fig.width=22, fig.height=14, message=FALSE, warning=FALSE, results=FALSE}
cpdb_luad_lusc |> filter(abs(log2_fc) > 1, fdr < 0.1,  cluster_1 == "Neutrophils" | cluster_2 == "Neutrophils") |> cpdb_plot(edge_color = "abs(log10(fdr))", edge_color_scale=scale_edge_color_viridis())
```
