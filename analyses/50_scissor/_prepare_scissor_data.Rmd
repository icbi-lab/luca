...
genentech_tpm = full_join(tpm_poplar, tpm_oak, by="gene_symbol")
```

```{r}
# bring metadata into the proper format for scissor
clinical_data_genentech = genentech_clinical |>
   select(sample_id = alias, study=STUDYNAME, type=HIST, time=OS_MONTHS, status=OS_CENSOR, gender, response=BCOR) |>
   # months to days
   ...
   mutate(response_binary = recode(response, PR=1, CR=1, PD=0))
```


```{r}
genentech_tpm_mat = genentech_tpm |> column_to_rownames("gene_symbol") 

write_rds(genentech_tpm_mat, "../../data/14_ici_treatment/Genentech_for_scissor/genentech_ici_treatment.rds")
