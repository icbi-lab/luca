...
tar cvf - --dereference --hard-dereference data/10_references data/13_tcga/for_scissor data/12_input_adatas data/11_own_datasets/{batch1_3patients,batch2_5patients,velocyto} | xz -9 -T 32 > data/zenodo/input_data.tar.xz

# intermediate results (required to run the second part of the workflow that doesn't depend on specialized hardware)
# tar cvf - --dereference --hard-dereference data/20_build_atlas | xz -9 -T 32 > data/zenodo/build_atlas_results.tar.xz
